...
def make_graph(molecule_name, gb_structure, gb_scalar_coupling, mc, ob_mc):
    ...
    bond_type_detail  = ["None"]*num_edge#np.zeros((num_edge,1), str)
    ...
    for i in range(num_atom):
        for j in range(num_atom):
            ...
            if bond is not None:
                ...
                # bond type detail
                bond_type_detail[ij] = (a[i] +"_"+ str(bond.GetBondType()) +"_" + a[j])
...
def do_one(p):
    i, molecule_name, gb_structure, gb_scalar_coupling, graph_file, mc, ob_mc = p
    g = make_graph(molecule_name, gb_structure, gb_scalar_coupling, mc, ob_mc)
...
def run_convert_to_graph():
    ...
    for i, molecule_name in enumerate(molecule_names):
        ...
        p = (i, molecule_name, gb_structure, 
                gb_scalar_coupling, graph_file, mc_gr, ob_mc_gr)
        param.append(p)
    if 1:
        for p in tqdm(param):
            do_one(p)
