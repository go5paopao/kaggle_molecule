...
class WL(object):
    ...
    def __init__(self, mol, depth, sep = None):
        ...
        self.sep = sep
        self.calc(depth)
        self.molecule_name = mol2_file.split('/')[-1].split('\\')[-1].split('.')[0]
    ...
    def to_arr(self):
        return np.array([self.labels[i] for i in range(len(self.m.GetAtoms()))])
...
def make_graph(molecule_name, gb_structure, gb_scalar_coupling, mc, ob_mc, scc_mean_and_std):
    ...
    distance_inv1   = np.zeros((num_edge,1),np.float32) #real
    distance_inv2   = np.zeros((num_edge,1),np.float32) #real
    distance_inv3   = np.zeros((num_edge,1),np.float32) #real
    distance_inv6   = np.zeros((num_edge,1),np.float32) #real
    ...
    for i in range(num_atom):
        for j in range(num_atom):
            ...
            distance_inv1[ij] = 1 / distance[ij]
            distance_inv2[ij] = 1 / (distance[ij]**2)
            distance_inv3[ij] = 1 / (distance[ij]**3)
            distance_inv6[ij] = 1 / (distance[ij]**6)
